...
float calculate_continuation_loss(GPT* gpt, unsigned short* tokens, int context_len, int total_len) {
    ...
    unsigned short* input = (unsigned short*)calloc(gpt->seq_len, sizeof(unsigned short));
    memcpy(input, tokens, total_len * sizeof(unsigned short));
    
    // Forward pass
    forward_pass_gpt(gpt, input);
    ...
    for (int i = context_len; i < total_len; i++) {
        ...
        float* logits = &gpt->output[(i - 1) * gpt->vocab_size];
        ...
        float sum_exp = 0.0f;
        float* probs = (float*)malloc(gpt->vocab_size * sizeof(float));
        for (int v = 0; v < gpt->vocab_size; v++) {
            probs[v] = expf(logits[v] - max_logit);
            sum_exp += probs[v];
        }
        
        // Cross-entropy for target token
        unsigned short target = tokens[i];
        float prob = probs[target] / sum_exp;
