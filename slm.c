...
void backward_pass_slm(SLM* slm, unsigned char* d_X) {
    ...
    for (int t = 0; t < slm->ssm2->seq_len; t++) {
        ...
        // ∂L/∂M1_t += B2^T (∂L/∂H2_t) - Gradient from state path
        ...
        // ∂L/∂M1_t += D2^T (∂L/∂Y2_t) - Gradient from output path
        CHECK_CUBLAS(cublasSgemm(slm->ssm2->cublas_handle,
                                CUBLAS_OP_N, CUBLAS_OP_N,
...
SLM* load_slm(const char* filename, int custom_batch_size) {
    ...
    CHECK_CUDA(cudaMalloc(&slm->d_mlp1_output, ssm1->seq_len * ssm1->batch_size * ssm1->output_dim * sizeof(float)));
    ...
    CHECK_CUDA(cudaMalloc(&slm->d_mlp1_gradients, ssm1->seq_len * ssm1->batch_size * ssm1->output_dim * sizeof(float)));
