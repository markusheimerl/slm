...
SLM* load_slm(const char* filename, int custom_batch_size) {
    ...
    SLM* slm = (SLM*)malloc(sizeof(SLM));
    slm->ssm = ssm;
    slm->vocab_size = 256; // Fixed vocab size for character-level model
    ...
    CHECK_CUDA(cudaMalloc(&slm->d_softmax, ssm->seq_len * ssm->batch_size * slm->vocab_size * sizeof(float)));
...
void generate_text_slm(SLM* slm, const char* seed_text, int generation_length, float temperature) {
    ...
    for (int i = 0; i < seed_len; i++) {
        ...
        CHECK_CUBLAS(cublasSgemm(slm->ssm3->cublas_handle,
                                ...
                                &alpha, slm->ssm3->d_C, slm->ssm3->state_dim,
                                d_o3_current, slm->ssm3->state_dim,
                                &beta, d_ssm3_output, slm->ssm3->output_dim));
