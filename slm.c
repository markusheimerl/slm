...
SLM* init_slm(int embed_dim, int state_dim, int seq_len, int batch_size) {
    ...
    // Initialize third SSM (embed_dim -> embed_dim)
    slm->ssm3 = init_ssm(embed_dim, state_dim, embed_dim, seq_len, batch_size);
    
    // Initialize fourth SSM (embed_dim -> embed_dim)
    ...
    CHECK_CUDA(cudaMalloc(&slm->d_ssm2_gradients, seq_len * batch_size * embed_dim * sizeof(float)));
    CHECK_CUDA(cudaMalloc(&slm->d_ssm3_gradients, seq_len * batch_size * embed_dim * sizeof(float)));
...
void free_slm(SLM* slm) {
    if (slm) {
        ...
        cudaFree(slm->d_ssm2_gradients);
        cudaFree(slm->d_ssm3_gradients);
...
void forward_pass_slm(SLM* slm, unsigned char* d_X) {
    ...
    // Third SSM layer: SSM2 output -> SSM3
    // H3_t = Y2_t B3^T + H3_{t-1} A3^T
    // O3_t = H3_t σ(H3_t)
    // Y3_t = O3_t C3^T + Y2_t D3^T
    reset_state_ssm(slm->ssm3);
    for (int t = 0; t < seq_len; t++) {
        float* d_ssm2_output_t = slm->d_ssm2_output + t * batch_size * slm->embed_dim;
        forward_pass_ssm(slm->ssm3, d_ssm2_output_t, t);
    }
    ...
    // Fourth SSM layer: SSM3 output -> SSM4
    // H4_t = Y3_t B4^T + H4_{t-1} A4^T
    // O4_t = H4_t σ(H4_t)
    // Y4_t = O4_t C4^T + Y3_t D4^T
    reset_state_ssm(slm->ssm4);
    for (int t = 0; t < seq_len; t++) {
        float* d_ssm3_output_t = slm->d_ssm3_output + t * batch_size * slm->embed_dim;
        forward_pass_ssm(slm->ssm4, d_ssm3_output_t, t);
    }

    // Z_t = Y4_t W_1
    // A_t = Z_t σ(Z_t)
    // L_t = A_t W_2 - Forward through MLP
    forward_pass_mlp(slm->mlp, slm->ssm4->d_predictions);
...
void backward_pass_slm(SLM* slm, unsigned char* d_X) {
    ...
    // ∂L/∂W_1 = Y4_t^T (∂L/∂Z_t) - Backward through MLP
    backward_pass_mlp(slm->mlp, slm->ssm4->d_predictions);
    
    // Copy MLP input gradients to fourth SSM output error
    int total_elements = slm->ssm4->seq_len * slm->ssm4->batch_size * slm->ssm4->output_dim;
    CHECK_CUDA(cudaMemcpy(slm->ssm4->d_error, slm->mlp->d_error, 
                         total_elements * sizeof(float), cudaMemcpyDeviceToDevice));
    
    // Backward through fourth SSM layer (SSM4)
    // ∂L/∂C4 = Σ_t (∂L/∂Y4_t)^T O4_t
    // ∂L/∂D4 = Σ_t (∂L/∂Y4_t)^T Y3_t  
    // ∂L/∂O4_t = (∂L/∂Y4_t)C4
    // ∂L/∂H4_t = ∂L/∂O4_t ⊙ [σ(H4_t) + H4_t σ(H4_t)(1-σ(H4_t))] + (∂L/∂H4_{t+1})A4
    // ∂L/∂A4 = Σ_t (∂L/∂H4_t)^T H4_{t-1}
    // ∂L/∂B4 = Σ_t (∂L/∂H4_t)^T Y3_t
    backward_pass_ssm(slm->ssm4, slm->d_ssm3_output);
    
    // Compute gradients with respect to SSM3 output (input to SSM4)
    // ∂L/∂Y3_t = (∂L/∂H4_t) B4 + (∂L/∂Y4_t) D4
    const float alpha = 1.0f;
    const float beta = 0.0f;
    
    CHECK_CUDA(cudaMemset(slm->d_ssm3_gradients, 0, 
                         slm->ssm4->seq_len * slm->ssm4->batch_size * slm->ssm4->input_dim * sizeof(float)));
    
    for (int t = 0; t < slm->ssm4->seq_len; t++) {
        float* d_ssm3_grad_t = slm->d_ssm3_gradients + t * slm->ssm4->batch_size * slm->ssm4->input_dim;
        float* d_state_error_t = slm->ssm4->d_state_error + t * slm->ssm4->batch_size * slm->ssm4->state_dim;
        float* d_output_error_t = slm->ssm4->d_error + t * slm->ssm4->batch_size * slm->ssm4->output_dim;
        
        // ∂L/∂Y3_t += B4^T (∂L/∂H4_t) - Gradient from state path
        CHECK_CUBLAS(cublasSgemm(slm->ssm4->cublas_handle,
                                CUBLAS_OP_N, CUBLAS_OP_N,
                                slm->ssm4->input_dim, slm->ssm4->batch_size, slm->ssm4->state_dim,
                                &alpha, slm->ssm4->d_B, slm->ssm4->input_dim,
                                d_state_error_t, slm->ssm4->state_dim,
                                &beta, d_ssm3_grad_t, slm->ssm4->input_dim));
        
        // ∂L/∂Y3_t += D4^T (∂L/∂Y4_t) - Gradient from output path
        CHECK_CUBLAS(cublasSgemm(slm->ssm4->cublas_handle,
                                CUBLAS_OP_N, CUBLAS_OP_N,
                                slm->ssm4->input_dim, slm->ssm4->batch_size, slm->ssm4->output_dim,
                                &alpha, slm->ssm4->d_D, slm->ssm4->input_dim,
                                d_output_error_t, slm->ssm4->output_dim,
                                &alpha, d_ssm3_grad_t, slm->ssm4->input_dim));
    }
    
    // Copy gradients to third SSM output error
    CHECK_CUDA(cudaMemcpy(slm->ssm3->d_error, slm->d_ssm3_gradients, 
                         total_elements * sizeof(float), cudaMemcpyDeviceToDevice));
    
    // Backward through third SSM layer (SSM3)
    ...
    backward_pass_ssm(slm->ssm3, slm->d_ssm2_output);
    
    // Compute gradients with respect to SSM2 output (input to SSM3)
    // ∂L/∂Y2_t = (∂L/∂H3_t) B3 + (∂L/∂Y3_t) D3
    CHECK_CUDA(cudaMemset(slm->d_ssm2_gradients, 0, 
                         slm->ssm3->seq_len * slm->ssm3->batch_size * slm->ssm3->input_dim * sizeof(float)));
    
    for (int t = 0; t < slm->ssm3->seq_len; t++) {
        float* d_ssm2_grad_t = slm->d_ssm2_gradients + t * slm->ssm3->batch_size * slm->ssm3->input_dim;
        float* d_state_error_t = slm->ssm3->d_state_error + t * slm->ssm3->batch_size * slm->ssm3->state_dim;
        float* d_output_error_t = slm->ssm3->d_error + t * slm->ssm3->batch_size * slm->ssm3->output_dim;
        
        // ∂L/∂Y2_t += B3^T (∂L/∂H3_t) - Gradient from state path
        CHECK_CUBLAS(cublasSgemm(slm->ssm3->cublas_handle,
                                CUBLAS_OP_N, CUBLAS_OP_N,
                                slm->ssm3->input_dim, slm->ssm3->batch_size, slm->ssm3->state_dim,
                                &alpha, slm->ssm3->d_B, slm->ssm3->input_dim,
                                d_state_error_t, slm->ssm3->state_dim,
                                &beta, d_ssm2_grad_t, slm->ssm3->input_dim));
        
        // ∂L/∂Y2_t += D3^T (∂L/∂Y3_t) - Gradient from output path
        CHECK_CUBLAS(cublasSgemm(slm->ssm3->cublas_handle,
                                CUBLAS_OP_N, CUBLAS_OP_N,
                                slm->ssm3->input_dim, slm->ssm3->batch_size, slm->ssm3->output_dim,
                                &alpha, slm->ssm3->d_D, slm->ssm3->input_dim,
                                d_output_error_t, slm->ssm3->output_dim,
                                &alpha, d_ssm2_grad_t, slm->ssm3->input_dim));
    }
    
    // Copy gradients to second SSM output error
    CHECK_CUDA(cudaMemcpy(slm->ssm2->d_error, slm->d_ssm2_gradients, 
                         total_elements * sizeof(float), cudaMemcpyDeviceToDevice));
...
SLM* load_slm(const char* filename, int custom_batch_size) {
    ...
    if (dot) *dot = '\0';
    strcat(ssm3_file, "_ssm3.bin");
    SSM* ssm3 = load_ssm(ssm3_file, custom_batch_size);
    if (!ssm3) {
        free_ssm(ssm1);
        free_ssm(ssm2);
        return NULL;
    ...
    if (dot) *dot = '\0';
    strcat(ssm4_file, "_ssm4.bin");
    SSM* ssm4 = load_ssm(ssm4_file, custom_batch_size);
    if (!ssm4) {
        free_ssm(ssm1);
        free_ssm(ssm2);
        free_ssm(ssm3);
    ...
    CHECK_CUDA(cudaMalloc(&slm->d_ssm2_output, ssm1->seq_len * ssm1->batch_size * ssm1->output_dim * sizeof(float)));
    CHECK_CUDA(cudaMalloc(&slm->d_ssm3_output, ssm1->seq_len * ssm1->batch_size * ssm1->output_dim * sizeof(float)));
    ...
    CHECK_CUDA(cudaMalloc(&slm->d_ssm2_gradients, ssm1->seq_len * ssm1->batch_size * ssm1->output_dim * sizeof(float)));
    CHECK_CUDA(cudaMalloc(&slm->d_ssm3_gradients, ssm1->seq_len * ssm1->batch_size * ssm1->output_dim * sizeof(float)));
...
void generate_text_slm(SLM* slm, const char* seed_text, int generation_length, float temperature) {
    ...
    // Copy third SSM weights
    CHECK_CUDA(cudaMemcpy(gen_slm->ssm3->d_A, slm->ssm3->d_A, 
                         slm->ssm3->state_dim * slm->ssm3->state_dim * sizeof(float), 
                         cudaMemcpyDeviceToDevice));
    CHECK_CUDA(cudaMemcpy(gen_slm->ssm3->d_B, slm->ssm3->d_B, 
                         slm->ssm3->state_dim * slm->ssm3->input_dim * sizeof(float), 
                         cudaMemcpyDeviceToDevice));
    CHECK_CUDA(cudaMemcpy(gen_slm->ssm3->d_C, slm->ssm3->d_C, 
                         slm->ssm3->output_dim * slm->ssm3->state_dim * sizeof(float), 
                         cudaMemcpyDeviceToDevice));
    CHECK_CUDA(cudaMemcpy(gen_slm->ssm3->d_D, slm->ssm3->d_D, 
                         slm->ssm3->output_dim * slm->ssm3->input_dim * sizeof(float), 
                         cudaMemcpyDeviceToDevice));
    
    // Copy fourth SSM weights
    CHECK_CUDA(cudaMemcpy(gen_slm->ssm4->d_A, slm->ssm4->d_A, 
                         slm->ssm4->state_dim * slm->ssm4->state_dim * sizeof(float), 
                         cudaMemcpyDeviceToDevice));
    CHECK_CUDA(cudaMemcpy(gen_slm->ssm4->d_B, slm->ssm4->d_B, 
                         slm->ssm4->state_dim * slm->ssm4->input_dim * sizeof(float), 
                         cudaMemcpyDeviceToDevice));
    CHECK_CUDA(cudaMemcpy(gen_slm->ssm4->d_C, slm->ssm4->d_C, 
                         slm->ssm4->output_dim * slm->ssm4->state_dim * sizeof(float), 
                         cudaMemcpyDeviceToDevice));
    CHECK_CUDA(cudaMemcpy(gen_slm->ssm4->d_D, slm->ssm4->d_D, 
                         slm->ssm4->output_dim * slm->ssm4->input_dim * sizeof(float), 
                         cudaMemcpyDeviceToDevice));
    ...
    reset_state_ssm(gen_slm->ssm3);
    reset_state_ssm(gen_slm->ssm4);
    ...
    for (int i = 0; i < seed_len; i++) {
        ...
        float* d_y2_t = gen_slm->ssm2->d_predictions + i * 1 * gen_slm->ssm2->output_dim;
        float* d_ssm2_out_t = gen_slm->d_ssm2_output + i * 1 * gen_slm->embed_dim;
        CHECK_CUDA(cudaMemcpy(d_ssm2_out_t, d_y2_t, 
                             gen_slm->embed_dim * sizeof(float), cudaMemcpyDeviceToDevice));
        
        // Forward through third SSM
        forward_pass_ssm(gen_slm->ssm3, d_ssm2_out_t, i);
        
        // Get the third SSM output for this timestep and copy to intermediate buffer
        float* d_y3_t = gen_slm->ssm3->d_predictions + i * 1 * gen_slm->ssm3->output_dim;
        float* d_ssm3_out_t = gen_slm->d_ssm3_output + i * 1 * gen_slm->embed_dim;
        CHECK_CUDA(cudaMemcpy(d_ssm3_out_t, d_y3_t, 
                             gen_slm->embed_dim * sizeof(float), cudaMemcpyDeviceToDevice));
        
        // Forward through fourth SSM
        forward_pass_ssm(gen_slm->ssm4, d_ssm3_out_t, i);
    }
    ...
    for (int i = 0; i < generation_length; i++) {
        ...
        float* d_ssm2_out_t = gen_slm->d_ssm2_output + timestep * 1 * gen_slm->embed_dim;
        CHECK_CUDA(cudaMemcpy(d_ssm2_out_t, d_y2_t, 
                             gen_slm->embed_dim * sizeof(float), cudaMemcpyDeviceToDevice));
        
        // Forward through third SSM
        forward_pass_ssm(gen_slm->ssm3, d_ssm2_out_t, timestep);
        
        // Get the third SSM output for this timestep and copy to intermediate buffer
        float* d_y3_t = gen_slm->ssm3->d_predictions + timestep * 1 * gen_slm->ssm3->output_dim;
        float* d_ssm3_out_t = gen_slm->d_ssm3_output + timestep * 1 * gen_slm->embed_dim;
        CHECK_CUDA(cudaMemcpy(d_ssm3_out_t, d_y3_t, 
                             gen_slm->embed_dim * sizeof(float), cudaMemcpyDeviceToDevice));
        
        // Forward through fourth SSM
        forward_pass_ssm(gen_slm->ssm4, d_ssm3_out_t, timestep);
        
        // Get the fourth SSM output for this timestep
        float* d_y4_t = gen_slm->ssm4->d_predictions + timestep * 1 * gen_slm->ssm4->output_dim;
        
        // Forward through MLP
        forward_pass_mlp(gen_slm->mlp, d_y4_t);
