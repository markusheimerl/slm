...
SLM* init_slm(int embed_dim, int state_dim, int seq_len, int batch_size) {
    ...
    // Initialize first MLP (embed_dim -> embed_dim)
    ...
    // Initialize second MLP (embed_dim -> vocab_size)
    ...
    CHECK_CUDA(cudaMalloc(&slm->d_mlp1_gradients, seq_len * batch_size * embed_dim * sizeof(float)));
...
void forward_pass_slm(SLM* slm, unsigned char* d_X) {
    ...
    // H2_t = L1_t B2^T + H2_{t-1} A2^T
    // O2_t = H2_t σ(H2_t)
    // Y2_t = O2_t C2^T + L1_t D2^T
    reset_state_ssm(slm->ssm2);
    for (int t = 0; t < seq_len; t++) {
        float* d_mlp1_output_t = slm->d_mlp1_output + t * batch_size * slm->embed_dim;
    ...
    // Z2_t = Y2_t W2_1
    // A2_t = Z2_t σ(Z2_t)
    // L2_t = A2_t W2_2 - Forward through final MLP
...
void backward_pass_slm(SLM* slm, unsigned char* d_X) {
    ...
    // ∂L/∂W2_2 = A2_t^T (∂L/∂L2_t)
    // ∂L/∂A2_t = (∂L/∂L2_t)(W2_2)^T
    ...
    CHECK_CUDA(cudaMemcpy(slm->ssm2->d_error, slm->mlp2->d_error, 
                         total_elements * sizeof(float), cudaMemcpyDeviceToDevice));
    
    // Backward through second SSM layer (SSM2)
    // ∂L/∂C2 = Σ_t (∂L/∂Y2_t)^T O2_t
    // ∂L/∂D2 = Σ_t (∂L/∂Y2_t)^T L1_t  
    // ∂L/∂O2_t = (∂L/∂Y2_t)C2
    // ∂L/∂H2_t = ∂L/∂O2_t ⊙ [σ(H2_t) + H2_t σ(H2_t)(1-σ(H2_t))] + (∂L/∂H2_{t+1})A2
    // ∂L/∂A2 = Σ_t (∂L/∂H2_t)^T H2_{t-1}
    // ∂L/∂B2 = Σ_t (∂L/∂H2_t)^T L1_t
    backward_pass_ssm(slm->ssm2, slm->d_mlp1_output);
    
    // Compute gradients with respect to MLP1 output (input to SSM2)
    // ∂L/∂L1_t = (∂L/∂H2_t) B2 + (∂L/∂Y2_t) D2
    const float alpha = 1.0f;
    const float beta = 0.0f;
    
    CHECK_CUDA(cudaMemset(slm->d_mlp1_gradients, 0, 
                         slm->ssm2->seq_len * slm->ssm2->batch_size * slm->ssm2->input_dim * sizeof(float)));
    
    for (int t = 0; t < slm->ssm2->seq_len; t++) {
        float* d_mlp1_grad_t = slm->d_mlp1_gradients + t * slm->ssm2->batch_size * slm->ssm2->input_dim;
        ...
        // ∂L/∂L1_t += B2^T (∂L/∂H2_t) - Gradient from state path
        CHECK_CUBLAS(cublasSgemm(slm->ssm2->cublas_handle,
                                ...
                                &beta, d_mlp1_grad_t, slm->ssm2->input_dim));
        
        // ∂L/∂L1_t += D2^T (∂L/∂Y2_t) - Gradient from output path
        CHECK_CUBLAS(cublasSgemm(slm->ssm2->cublas_handle,
                                CUBLAS_OP_N, CUBLAS_OP_N,
...
SLM* load_slm(const char* filename, int custom_batch_size) {
    ...
    if (dot) *dot = '\0';
    strcat(mlp1_file, "_mlp1.bin");
    MLP* mlp1 = load_mlp(mlp1_file, ssm1->seq_len * ssm1->batch_size);
    if (!mlp1) {
        free_ssm(ssm1);
        return NULL;
    }
    ...
    if (dot) *dot = '\0';
    strcat(mlp2_file, "_mlp2.bin");
    MLP* mlp2 = load_mlp(mlp2_file, ssm1->seq_len * ssm1->batch_size);
    if (!mlp2) {
        free_ssm(ssm1);
        free_mlp(mlp1);
        free_ssm(ssm2);
    ...
    slm->ssm2 = ssm2;
    slm->mlp2 = mlp2;
    slm->vocab_size = mlp2->output_dim;
    ...
    CHECK_CUDA(cudaMalloc(&slm->d_mlp1_output, ssm1->seq_len * ssm1->batch_size * slm->embed_dim * sizeof(float)));
    ...
    CHECK_CUDA(cudaMalloc(&slm->d_mlp1_gradients, ssm1->seq_len * ssm1->batch_size * slm->embed_dim * sizeof(float)));
...
void generate_text_slm(SLM* slm, const char* seed_text, int generation_length, float temperature) {
    ...
    for (int i = 0; i < seed_len; i++) {
        ...
        // Get the first MLP output for this timestep and copy to intermediate buffer
        float* d_mlp1_out_t = gen_slm->d_mlp1_output + i * 1 * gen_slm->embed_dim;
        CHECK_CUDA(cudaMemcpy(d_mlp1_out_t, gen_slm->mlp1->d_predictions, 
                             gen_slm->embed_dim * sizeof(float), cudaMemcpyDeviceToDevice));
        
        // Forward through second SSM
        forward_pass_ssm(gen_slm->ssm2, d_mlp1_out_t, i);
    }
