...
SLM* init_slm(int embed_dim, int state_dim, int seq_len, int num_layers, int batch_size) {
    ...
    // Initialize layers (embed_dim -> embed_dim for intermediate, embed_dim -> vocab_size for final)
    slm->ssms = (SSM**)malloc(num_layers * sizeof(SSM*));
    
    for (int i = 0; i < num_layers; i++) {
        // Last layer outputs to vocab_size, others to embed_dim
        int ssm_output_dim = (i == num_layers - 1) ? slm->vocab_size : embed_dim;
        slm->ssms[i] = init_ssm(embed_dim, state_dim, ssm_output_dim, seq_len, batch_size);
    }
...
void backward_pass_slm(SLM* slm, unsigned char* d_X) {
    ...
    for (int layer = slm->num_layers - 1; layer >= 0; layer--) {
        ...
        if (layer > 0) {
            ...
            // Copy computed gradients to previous layer's SSM error buffer for next iteration
...
void generate_text_slm(SLM* slm, const char* seed_text, int generation_length, float temperature, float top_p) {
    ...
    for (int i = 0; i < seed_len; i++) {
        ...
        for (int layer = 0; layer < gen_slm->num_layers; layer++) {
            ...
            if (layer < gen_slm->num_layers - 1) {
                float* d_ssm_out_t = gen_slm->ssms[layer]->d_predictions;
    ...
    for (int i = 0; i < generation_length; i++) {
        ...
        float* d_final_logits = gen_slm->ssms[gen_slm->num_layers - 1]->d_predictions;
