...
SLM* init_slm(int embed_dim, int state_dim, int seq_len, int batch_size) {
    ...
    // Initialize third SSM (embed_dim -> vocab_size)
    slm->ssm3 = init_ssm(embed_dim, state_dim, slm->vocab_size, seq_len, batch_size);
...
void forward_pass_slm(SLM* slm, unsigned char* d_X) {
    ...
    for (int t = 0; t < seq_len; t++) {
        float* d_ssm2_output_t = slm->ssm2->d_predictions + t * batch_size * slm->embed_dim;
        forward_pass_ssm(slm->ssm3, d_ssm2_output_t, t);
    }
    
    // P_t = softmax(Y3_t) - Apply softmax for probability distribution
    int total_tokens = seq_len * batch_size;
    int blocks = (total_tokens + 255) / 256;
    softmax_kernel<<<blocks, 256>>>(
        slm->d_softmax, slm->ssm3->d_predictions, total_tokens, slm->vocab_size
    );
}
...
float calculate_loss_slm(SLM* slm, unsigned char* d_y) {
    ...
    // ∂L/∂Y3_t = P_t - 1_{y_t} - Compute cross-entropy gradient (softmax - one_hot) for backprop
    int blocks = (total_tokens + 255) / 256;
    cross_entropy_gradient_kernel<<<blocks, 256>>>(
        slm->ssm3->d_error, slm->d_softmax, d_y, total_tokens, slm->vocab_size
    );
...
SLM* load_slm(const char* filename, int custom_batch_size) {
    ...
    slm->ssm1 = ssm1;
    slm->ssm2 = ssm2;
    slm->vocab_size = 256; // Should match from initialization
...
void generate_text_slm(SLM* slm, const char* seed_text, int generation_length, float temperature) {
    ...
    for (int i = 0; i < seed_len; i++) {
        ...
        forward_pass_ssm(gen_slm->ssm3, d_y2_t, i);
    }
    ...
    for (int i = 0; i < generation_length; i++) {
        ...
        // Forward through third SSM
        forward_pass_ssm(gen_slm->ssm3, d_y2_t, timestep);
        ...
        softmax_kernel<<<1, 256>>>(
            gen_slm->d_softmax, d_y3_t, 1, gen_slm->vocab_size
        );
