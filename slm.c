...
void forward_pass_slm(SLM* slm, unsigned char* d_X_t, int timestep) {
    ...
    float* d_embedded_t = slm->d_embedded_input + timestep * batch_size * slm->embed_dim;
    ...
    float* current_input = d_embedded_t;
    ...
    float* d_softmax_t = slm->d_softmax + timestep * batch_size * slm->vocab_size;
...
void backward_pass_slm(SLM* slm, unsigned char* d_X_t, int timestep) {
    ...
    for (int layer = slm->num_layers - 1; layer >= 1; layer--) {
        ...
        float* layer_input_t = slm->ssms[layer - 1]->d_layer2_output + timestep * batch_size * slm->embed_dim;
        
        // Backward SSM of current layer for this timestep
        backward_pass_ssm(slm->ssms[layer], layer_input_t, timestep);
        
        // Compute input gradients
        float* input_grad_t = slm->d_input_gradients + timestep * batch_size * slm->embed_dim;
        float* error_hidden_t = slm->ssms[layer]->d_error_hidden + timestep * batch_size * slm->ssms[layer]->state_dim;
        float* error_output_t = slm->ssms[layer]->d_error_output + timestep * batch_size * slm->ssms[layer]->output_dim;
        
        // ∂L/∂X = B^T (∂L/∂H) + D^T (∂L/∂Y)
        CHECK_CUBLAS(cublasSgemm(slm->cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N,
        ...
        float* prev_error_output_t = slm->ssms[layer - 1]->d_error_output + timestep * batch_size * slm->embed_dim;
        CHECK_CUDA(cudaMemcpy(prev_error_output_t, input_grad_t, 
                             batch_size * slm->embed_dim * sizeof(float), cudaMemcpyDeviceToDevice));
    }
    
    float* layer_input_t = slm->d_embedded_input + timestep * batch_size * slm->embed_dim;
    backward_pass_ssm(slm->ssms[0], layer_input_t, timestep);
    
    // Compute embedding gradients from first layer
    float* embed_grad_t = slm->d_input_gradients + timestep * batch_size * slm->embed_dim;
    float* error_hidden_t = slm->ssms[0]->d_error_hidden + timestep * batch_size * slm->ssms[0]->state_dim;
    float* error_output_t = slm->ssms[0]->d_error_output + timestep * batch_size * slm->ssms[0]->output_dim;
...
void generate_text_slm(SLM* slm, const char* seed_text, int generation_length, float temperature, float top_p) {
    ...
    for (int i = 0; i < seed_len + generation_length; i++) {
        ...
        if (i >= seed_len) {
            ...
            float* d_probs = gen_slm->d_softmax + i * gen_slm->vocab_size;
